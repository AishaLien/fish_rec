...
# 此实验将训练集和测试集微同一个集合  
train_whole_sample_size = 2117 # 训练集总量  
test_whole_sample_size = 2117  # 测试集总量  
gesture_class = 3  # 分类类别个数  
train_batch_size = 100  # 训练集每个批次的样本个数  
test_batch_size = 100  # 测试集每个批次的样本个数  
  
image_size = 28  # 样本size ：28*28  
# 训练集.tfrecords 路径  
train_path = "train_data"  
# tensorboard的graph文件 保存路径  
graph_path = "my_graph/my_gra"  
# CNN模型文件 保存路径  
...
# 28*28图片单通道数据输入  
# 第一次卷积后图片size:28*28，输出32个feature map  
# 第一次池化后变为size:14*14，仍为 32个feature map  
# 第二次卷积后仍为14*14，输出的64个 feature map  
# 第二次池化后为7*7 仍为 64个 feature map  
# 7*7*64  
  
# 全连接层 1  
with tf.name_scope('Fc1'):  
    # 初始化第一个全连接层权值  
    W_fc1 = weight_variable([7 * 7 * 64, 1024], 'W_fc1')  # 因为输入为 64张 size: 7*7 feature map，定义全连接层有1024个神经元  
...
with tf.name_scope('Fc2'):  
    # 第二个全连接层  
    W_fc2 = weight_variable([1024, gesture_class], 'W_fc2')  
